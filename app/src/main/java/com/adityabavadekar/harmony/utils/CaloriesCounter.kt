...
class CaloriesCounter() {
    ...
    fun increment(newDistanceMeters: Double, durationSec: Double) {
        count += CaloriesCalculator.calculateCaloriesBurned(
            workoutType = WorkoutTypes.TYPE_UNKNOWN,
            durationSeconds = durationSec,
            distanceInMeters = newDistanceMeters,
            weightKg = userWeightKg ?: 0.0
        )
    }
